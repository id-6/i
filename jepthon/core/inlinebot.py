...
def main_menu():
    text = f"**- [بيثون بيثون](https://t.me/Bethon2)\
        \n المسـاعد\
        \n\
        \n◽ بيثون لـ {mention}**"
    buttons = [
...
async def inline_handler(event):  # sourcery no-metrics
    ...
    if query_user_id == Config.OWNER_ID or query_user_id in Config.SUDO_USERS:
        ...
        if query.startswith("jepiq"):
            ...
            result = builder.article(
                title="رسـالة تـرول من سـورس بيثون 🧸♥",
            ...
            result = builder.article(
                title="همسـة سـرية من سـورس بيثون 🧸♥",
                text=f"فقـط {razan} هـو مـن يستـطيع مشـاهدتهـا !",
                buttons=buttons,
            )
            await event.answer([result] if result else None)
            ...
            result = builder.article(
                title="رسـالة مخفيـة من سـورس بيثون 🧸♥",
                text=f"✖✖✖",
                buttons=buttons,
            )
            await event.answer([result] if result else None)
        ...
        buttons = [
            (
                Button.url("قنـاة السـورس ⚒️", "https://t.me/Bethon2"),
                Button.url(
                    "كـروب المـساعدة 📬",
                    "https://t.me/Bethon2",
                ),
        ...
        text, msg_entities = await event.client._parse_message_text(
            "**[بـوت بيثون 🧸♥](https://t.me/Jepthon)**\
            \n\
            \n❤ بيثون هـو بـوت بسـيط يدخـل الـى حسـابك لجعـلك تتحـكم به معـ اوامـر معينـة.\
            \n\
            \n**اذا كـنت تـرغب بتـنصيـب السـورس عـلى حسـابك ,\
            \n🐾 تعال الـى [كروب المسـاعدة](https://t.me/Bethon2)!**",
            "md",
        )
        result = types.InputBotInlineResult(
            id=str(uuid4()),
            type="photo",
            title="jepiq 🧸♥",
            description="ادخـل قناة المسـاعدة",
            url="https://t.me/@Bethon2",
            thumb=photo,
            content=photo,
            send_message=types.InputBotInlineMessageMediaAuto(
                reply_markup=markup, message=text, entities=msg_entities
